...
export function startDaemon(targetDir?: string): { pid: number; root: string } {
  ...
  if (!fs.existsSync(DAEMON_SCRIPT)) {
    throw new Error('Daemon not found. Reinstall klair.');
  }
...
export function openWebGui(): void {
...
export function resetSession(): void {
  ...
  for (const p of [path.join(KLAIR_DIR, 'camera.db'), STATE_PATH]) {
